...
class ReplayBuffer(object):
    ...
    def __init__(self, max_capacity, obs_shape, act_shape):
        ...
        self.next_obs = np.zeros((self.max_capacity, *self.obs_shape), dtype=np.float)
        ...
        self.dones = np.zeros((self.max_capacity, 1), dtype=np.float)

    def make_index(self, batch_size):
        return np.random.choice(self.get_size(), batch_size, replace=False)
    ...
    def add(self, obs, act, rew, new_obs, done):
        ...
        self.next_obs[self.pointer] = new_obs
